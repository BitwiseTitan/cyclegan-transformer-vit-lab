"""
Q3 training entrypoint for pretrained ViT fine-tuning.
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--run_name", type=str, default=f"vit_pretrained_{_timestamp()}")
...
def main() -> None:
    ...
    print(f"[q3][vit-pre] device={device}")
    print(f"[q3][vit-pre] run_dir={run_dir}")
    print(f"[q3][vit-pre] model_name={args.model_name}")

    # Pretrained ViT expects 224x224 input; we upsample CIFAR-10.
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        model.train()
        train_losses = []
        pbar = tqdm(train_loader, desc=f"q3 vit-pre epoch {epoch}", leave=True)
        for x, y in pbar:
            x = x.to(device)
            y = y.to(device)
        ...
        if test_acc > best_acc:
            ...
            print(f"[q3][vit-pre] new best checkpoint: {ckpt_dir / 'best.pt'} (acc={best_acc:.4f})")

        print(f"[q3][vit-pre][{epoch:03d}/{args.epochs}] train_loss={train_loss:.4f} test_loss={test_loss:.4f} test_acc={test_acc:.4f}")

    torch.save({"epoch": args.epochs, "state_dict": model.state_dict(), "test_acc": curves["test_acc"][-1]}, ckpt_dir / "last.pt")
    save_curves(run_dir / "curves.png", {"train_loss": curves["train_loss"], "test_loss": curves["test_loss"], "test_acc": curves["test_acc"]}, title="Q3 ViT (Pretrained) Curves")
    ...
    try:
        ...
        plt.title("Pretrained ViT Confusion Matrix (CIFAR-10)")
        plt.xlabel("Pred")
        plt.ylabel("True")
        plt.tight_layout()
