"""
Q3 training entrypoint for custom ViT.
"""
...
from q3_vit_custom import Q3ViT, Q3ViTConfig
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--run_name", type=str, default=f"vit_custom_{_timestamp()}")
...
def main() -> None:
    ...
    print(f"[q3][vit] device={device}")
    print(f"[q3][vit] run_dir={run_dir}")
    ...
    vit_cfg = Q3ViTConfig(patch_size=args.patch_size, embed_dim=args.embed_dim, depth=args.depth, num_heads=args.num_heads)
    model = Q3ViT(vit_cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        model.train()
        train_losses = []
        pbar = tqdm(train_loader, desc=f"q3 vit epoch {epoch}", leave=True)
        for x, y in pbar:
            x = x.to(device)
            y = y.to(device)
        ...
        if test_acc > best_acc:
            ...
            print(f"[q3][vit] new best checkpoint: {ckpt_dir / 'best.pt'} (acc={best_acc:.4f})")

        print(f"[q3][vit][{epoch:03d}/{args.epochs}] train_loss={train_loss:.4f} test_loss={test_loss:.4f} test_acc={test_acc:.4f}")

    torch.save({"epoch": args.epochs, "state_dict": model.state_dict(), "test_acc": curves["test_acc"][-1]}, ckpt_dir / "last.pt")
    save_curves(run_dir / "curves.png", {"train_loss": curves["train_loss"], "test_loss": curves["test_loss"], "test_acc": curves["test_acc"]}, title="Q3 ViT (Custom) Curves")
    ...
    try:
        ...
        plt.title("Custom ViT Confusion Matrix (CIFAR-10)")
        plt.xlabel("Pred")
        plt.ylabel("True")
        plt.tight_layout()
